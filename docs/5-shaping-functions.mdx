...
In [chapter 5 of the Book of Shaders](https://thebookofshaders.com/05/) we are introduced to the essential [kata](https://en.wikipedia.org/wiki/Kata) of shaders, **shaping functions**. These are functions as in C programming lagugage-like functions, but they are also functions as in mathematical _pure functions_. As pure functions, given the same input they will always give the same output. (This includes shaping functions that include pseudo-random processes, if the RNG is always same-seeded.) We use these functions in our shaders to "shape" features in like pixel colour across across the shader, or movement within the shader over time. GLSL includes a number of built-in shaping functions. The built-in [smoothstep](https://thebookofshaders.com/glossary/?search=smoothstep) is explored below. We can also build our own custom shaping functions through the compositino of built-in math functions like sin, abs, sign, clamp, min, max, mod, fract, ceil, floor, etc.
...
The function was plotted using the `smoothstep` trick show above. To produce the gradient the x position of each pixel is used as input to to produce a black to white colour intensity:
...
University is 20 years back for me, so I'd completely forgotten the subtlies of sin. Spent 20 minutes messing with [this interactive sin plot](https://www.desmos.com/calculator/w9jrdpvsmk) to relearn some trig fundamentals.
...
The next shader I attemped was quite ambitious. It's an animated gallery of shaping functions, where each function is use to generate:
...
- Text Rendering
- 2D Signed Distance Circle Easing
